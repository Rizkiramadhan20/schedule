...
export default function useManagementProyek() {
    ...
    // Handle input change
    const handleInputChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement> | { target: { name: string; value: string | number | string[]; type?: string } }) => {
    ...
    const addProyek = async () => {
        ...
        try {
            ...
            await addDoc(collection(db, process.env.NEXT_PUBLIC_COLLECTIONS_PROYEK as string), {
                ...form,
    ...
    const editProyek = async () => {
        ...
        try {
            ...
            await updateDoc(doc(db, process.env.NEXT_PUBLIC_COLLECTIONS_PROYEK as string, editId), {
                ...form,
                category: categoryName, // Simpan name, bukan id
                framework: frameworkArr, // Simpan array of object
                thumbnail: thumbnailUrl,
    ...
    const deleteProyek = async () => {
        ...
        setLoading(true);
        try {
            await deleteDoc(doc(db, process.env.NEXT_PUBLIC_COLLECTIONS_PROYEK as string, deleteId));
    ...
    const handleEdit = (proyek: Proyek) => {
        ...
        const categoryId = categories.find(cat => cat.name === proyek.category)?.id || '';
    ...
    const updateLink = (idx: number, field: 'label' | 'url', value: string) => {
        setForm(f => {
            ...
            newLinks[idx] = { ...newLinks[idx], [field]: value, updatedAt: Timestamp.now() };
    ...
    const updateDeposit = (idx: number, field: 'label' | 'price' | 'percent', value: string | number) => {
        setForm(f => {
